  ...
  ALPAKA_FN_ACC void operator()(const T_Acc& acc, Data *vec, T elements) const {
    ...
    for (T index: cms::alpakatools::elements_with_stride<T, T_Acc>(acc, elements)) {
      for (int i = 0; i < 200000; i++) {
          alpaka::atomicMin(acc, &s, b, alpaka::hierarchy::Blocks{});
          alpaka::atomicMin(acc, &s, c, alpaka::hierarchy::Blocks{});
      }
      alpaka::atomicMin(acc, &s, b, alpaka::hierarchy::Blocks{});
    }
...
struct global_block {
  ...
  ALPAKA_FN_ACC void operator()(const T_Acc& acc, Data *vec, T elements) const {
    ...
    for (T index: cms::alpakatools::elements_with_stride<T, T_Acc>(acc, elements)) {
      for (int i = 0; i < 200000; i++) {
          alpaka::atomicMin(acc, &vec[blockIdxInGrid], b, alpaka::hierarchy::Grids{});
          alpaka::atomicMin(acc, &vec[blockIdxInGrid], c, alpaka::hierarchy::Grids{});
      }
      alpaka::atomicMin(acc, &vec[blockIdxInGrid], b, alpaka::hierarchy::Grids{});
    }    
  }
...
struct global_grid {
  ...
  ALPAKA_FN_ACC void operator()(const T_Acc& acc, Data *vec, T elements) const {
    ...
    for (T index: cms::alpakatools::elements_with_stride<T, T_Acc>(acc, elements)) {  
      for (int i = 0; i < 200000; i++) {
          alpaka::atomicMin(acc, &vec[0], b, alpaka::hierarchy::Grids{}); //alpaka::hierarchy::Blocks/Threads/Grids
          alpaka::atomicMin(acc, &vec[0], c, alpaka::hierarchy::Grids{});
      }
      alpaka::atomicMin(acc, &vec[0], b, alpaka::hierarchy::Grids{});
    }    
  }
};
...
struct shared_grid {
  ...
  ALPAKA_FN_ACC void operator()(const T_Acc& acc, Data *vec, T elements) const {
    ...
    for (T index: cms::alpakatools::elements_with_stride<T, T_Acc>(acc, elements)) {
      for (int i = 0; i < 200000; i++) {
        alpaka::atomicMin(acc, &s, b, alpaka::hierarchy::Blocks{}); //alpaka::hierarchy::Blocks/Threads/Grids
        alpaka::atomicMin(acc, &s, c, alpaka::hierarchy::Blocks{});
      }
        alpaka::atomicMin(acc, &s, b, alpaka::hierarchy::Blocks{});
    }
    ...
    if (threadIdxLocal == 0) {
      alpaka::atomicMin(acc, &vec[0], s, alpaka::hierarchy::Grids{});
    }    
  }
};
